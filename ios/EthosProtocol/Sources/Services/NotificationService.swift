...
final class NotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    func registerNotificationCategories() {
        ...
        // Snooze actions for expiry notifications — reschedule reminder without user interaction
        let snooze7DaysAction = UNNotificationAction(identifier: "SNOOZE_7_DAYS", title: "Snooze 7 days", options: [])
        let snooze14DaysAction = UNNotificationAction(identifier: "SNOOZE_14_DAYS", title: "Snooze 14 days", options: [])

        // Primary check-in category with multiple actions
        let category = UNNotificationCategory(
            identifier: "CHECK_IN",
            actions: [checkInAction, snooze7DaysAction, snooze14DaysAction],
            intentIdentifiers: [],
